...
namespace Grauenwolf.TravellerTools.TradeCalculator
{
    ...
    {
        static ImmutableList<TradeGood> s_TradeGoods;
        static ImmutableList<TradeGood> s_LegalTradeGoods;

        public static void SetDataPath(string dataPath)
        {
            var file = new FileInfo(Path.Combine(dataPath, "TradeGoods.xml"));
            var converter = new XmlSerializer(typeof(TradeGoods));
            using (var stream = file.OpenRead())
                s_TradeGoods = ((TradeGoods)converter.Deserialize(stream)).TradeGood.ToImmutableList();

            s_LegalTradeGoods = s_TradeGoods.Where(g => g.Legal).ToImmutableList();
        }



        public static async Task<ManifestCollection> BuildManifestsAsync(int sectorX, int sectorY, int hexX, int hexY, int maxJumpDistance, bool advancedMode, bool illegalGoods, int brokerScore)
        {
            var random = new Dice();

            var worlds = await TravellerMapService.WorldsNearAsync(sectorX, sectorY, hexX, hexY, maxJumpDistance).ConfigureAwait(false);
            var result = await TradeEngine.BuildManifestsAsync(worlds, random, illegalGoods).ConfigureAwait(false);
        ...
        private static int CalculateBerthignCost(World origin, Dice random)
        {
        ...
        static async Task<ManifestCollection> BuildManifestsAsync(List<World> worlds, Dice random, bool illegalGoods)
        {
        ...
        public static async Task<Manifest> BuildManifestAsync(World origin, World destination, Dice random, bool illegalGoods)
        {
        ...
        public static TradeList BuildTradeList(World origin, bool advancedMode, bool illegalGoods, int brokerScore)
        {
            IReadOnlyList<TradeGood> goods;
            if (!illegalGoods)
                goods = s_LegalTradeGoods;
        ...
        private static void AddTradeGood(World origin, Dice random, IList<TradeOffer> result, TradeGood good, bool advancedMode, int brokerScore)
        {
        ...
        static decimal PurchasePriceModifier(Dice random, int purchaseBonus, int brokerScore, out int roll)
        {
        ...
        static decimal SalePriceModifier(Dice random, int saleBonus, int brokerScore, out int roll)
        {
        ...
        public static async Task<PassengerList> PassengersAsync(World origin, World destination, Dice random)
        {
        ...
        static async Task<Passenger> PassengerDetailAsync(Dice random, string travelType)
        {
        ...
        static public string PassengerTrait(Dice random, ref bool isPatron)
        {
        ...
        public static FreightList Freight(World origin, World destination, Dice random)
        {
            ...
            {
                var good = random.Choose(s_LegalTradeGoods);
                var detail = good.ChooseRandomDetail(random);
