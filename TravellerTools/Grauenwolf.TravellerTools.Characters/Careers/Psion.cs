...
namespace Grauenwolf.TravellerTools.Characters.Careers
{
	...
	{
		public Psion(string assignment, Book book) : base("Psion", assignment, book)
		{
		}
		...
		internal override void Mishap(Character character, Dice dice)
		{
			switch (dice.D(6))
			{
				...
				case 2:
					character.AddHistory("Telepathically contact something dangerous. Suffer from persistent and terrifying nightmares.");
					character.Psi -= 1;
					return;

				case 3:
					character.AddHistory("An anti-psi cult or gang attempts to expose or attack you.");
					switch (dice.D(3))
					{
						case 1:
				...
				case 4:
					...
					if (dice.D(2) == 1)
					{
						character.AddHistory("Use your psionic powers in an unethical fashion. Gain an enemy.");
						character.NextTermBenefits.MusterOut = false;
					}
					else
					{
						character.AddHistory("Refused to use your psionic powers in an unethical fashion.");
					}
					return;

				case 5:
					character.AddHistory("Experimented on by a corporation, government, or other organisation");
					return;

				case 6:
					character.AddHistory("Gift causes a former ally to turn on you and betray you. One Ally or Contact becomes an Enemy.");
					return;
			}
		}
		...
		protected void AttemptTalent(Character character, Dice dice, string name)
		{
			...
			{
				character.AddHistory("Learned " + name);
