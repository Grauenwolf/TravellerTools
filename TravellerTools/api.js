...
//Example: http://namey.muffinlabs.com/name.json?frequency=all&count=1000&with_surname=true
...
namey = {
	...
	jsonP: (function () {
		...
		function e(j) {
			...
			if (!c) { c = document.getElementsByTagName("head")[0] } c.appendChild(i)
		} function g(h, j, k) {
			f = "?";
			j = j || {};
			for (b in j) { if (j.hasOwnProperty(b)) { f += encodeURIComponent(b) + "=" + encodeURIComponent(j[b]) + "&" } } var i = "json" + (++a);
			d[i] = function (l) {
				k(l);
				try { delete d[i] } catch (m) { } d[i] = null;
			};
			e(h + f + "callback=" + i);
			return i
		} return { get: g }
 ...
 ;
 => ["John Cook", "Ruth Fisher", "Donna Collins"]
     *
